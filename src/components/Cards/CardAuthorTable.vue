	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{ padding: 0, }">
		...
		<a-table :columns="columns" :data-source="data" :pagination="false">
			...
			<template #status="{ status }">
				...
				>{{ status ? "ONLINE" : "OFFLINE" }}</a-tag>
			</template>

			<template #editBtn="{ text: row }">
				<a-button type="link" :data-id="row.key" class="btn-edit">Edit</a-button>
			</template>
		</a-table>
	</a-card>
	<!-- / Authors Table Card -->
